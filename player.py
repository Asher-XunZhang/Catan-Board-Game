class Player:
    ...
    def send_trade(self, other_player, bricks_val, ore_val, grain_val, lumber_val, wool_val):
        if (
                bricks_val <= self.bricks or ore_val <= self.ore or grain_val <= self.grain or lumber_val <= self.lumber or wool_val <= self.wool):
            if (
                    bricks_val <= other_player.bricks or ore_val <= other_player.ore or grain_val <= other_player.grain or lumber_val <= other_player.lumber or wool_val <= other_player.wool):
